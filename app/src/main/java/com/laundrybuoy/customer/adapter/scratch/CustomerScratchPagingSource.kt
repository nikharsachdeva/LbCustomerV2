...
class CustomerScratchPagingSource(private val customerApi: CustomerAPI) : PagingSource<Int, CustomerTransactionModel.Data.Partner>() {

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, CustomerTransactionModel.Data.Partner> {
        return try {
            val position = params.key ?: 1
            val response = customerApi.getCustomerScratchCard(position)
