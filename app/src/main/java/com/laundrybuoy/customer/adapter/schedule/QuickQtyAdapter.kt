...
class QuickQtyAdapter(val clickListener : QuantitySelectListener ) :
    ListAdapter<OrderQtyModel.OrderQtyModelItem, QuickQtyAdapter.AddressViewHolder>(
        ComparatorDiffUtil()
    ) {
    ...
    inner class AddressViewHolder(private val binding: RowItemSelectQtyBinding) :
        ...
        fun bind(qty: OrderQtyModel.OrderQtyModelItem, position: Int) {
            ...
            binding.qtyRootRl.setOnClickListener {
                ...
                clickListener.onQuantitySelected(qty)
            }

        }
    }

    interface QuantitySelectListener{
        fun onQuantitySelected(qty: OrderQtyModel.OrderQtyModelItem)
    }
